...
class StorySeoController extends Controller
{
    public function index()
    {
        $seo = SeoStory::first();
        return view ('admin.seo.storyseo', compact('seo'));
    }

    public function store()
    {
        $seo = SeoStory::create($this->validateData());
        $this->storeCoverImage($seo);

        if($seo)
        {
            return redirect()->back()->with('success', 'STORY SEO INFO CREATED!');
        }else{
    ...
    public function update()
    {
        ...
        if($seo)
        {
            return redirect()->back()->with('success', 'STORY SEO INFO UPDATED');
        }else{
