...
class SearchController extends Controller
{
    public function search(Request $request)
    {
        ...
        // TUTORIAL
        $tutorials = Tutorial::where('title','LIKE', "%{$search}%")->where('status', 1)->latest()->get();
        if (count($tutorials) == 0) {
            $category = CategoryTutorial::where('name','LIKE', "%{$search}%")->where('status', 1)->first();
            if($category) {
                $tutorials = $category->tutorial()->where('status', 1)->latest()->get();
            }else{
                $tag = TagTutorial::where('name','LIKE',"%{$search}%")->where('status', 1)->first();
                if($tag) {
                    $tutorials =  $tag->tutorial()->where('status', 1)->latest()->get();
                }
            }
        }

        // STORY
        $stories = Story::where('title','LIKE', "%{$search}%")->where('status', 1)->latest()->get();
        if (count($stories) == 0) {
            $category = CategoryStory::where('name','LIKE', "%{$search}%")->where('status', 1)->first();
            if($category) {
                $stories = $category->story()->where('status', 1)->latest()->get();
            }
        }

        // TEMPLATE
        $templates = Template::where('title','LIKE', "%{$search}%")->where('status', 1)->latest()->get();
        if (count($templates) == 0) {
            $category = CategoryTemplate::where('name','LIKE', "%{$search}%")->where('status', 1)->first();
            if($category) {
                $templates = $category->template()->where('status', 1)->latest()->get();
            }else{
                $tag = TagTemplate::where('name','LIKE',"%{$search}%")->where('status', 1)->first();
                if($tag) {
                    $templates =  $tag->template()->where('status', 1)->latest()->get();
                }
            }
        }

        // MOVIE
        $movies = Movie::where('name','LIKE', "%{$search}%")->where('status', 1)->latest()->get();
        if (count($movies) == 0) {
            $category = CategoryMovie::where('name','LIKE', "%{$search}%")->where('status', 1)->first();
            if($category) {
                $movies = $category->movie()->where('status', 1)->latest()->get();
            }
        }

        // YOUTUBE MOVIE
        $ytmovies = Youtubemovie::where('name','LIKE', "%{$search}%")->where('status', 1)->latest()->get();
        if (count($ytmovies) == 0) {
            $category = CategoryMovie::where('name','LIKE', "%{$search}%")->where('status', 1)->first();
            if($category) {
                $ytmovies = $category->ytmovie()->where('status', 1)->latest()->get();
            }
        }

        // SOURCES
        $sources = PreemiumFree::where('title','LIKE', "%{$search}%")->where('status', 1)->latest()->get();

        // PDDF
        $pdfs = PDF::where('name','LIKE', "%{$search}%")->orWhere('title','LIKE', "%{$search}%")->where('status', 1)->latest()->get();
        if (count($pdfs) == 0) {
            $category = CategoryPdf::where('name','LIKE', "%{$search}%")->where('status', 1)->first();
            if($category) {
                $pdfs = $category->pdf()->where('status', 1)->latest()->get();
            }else{
                $author = AuthorPdf::where('name','LIKE',"%{$search}%")->where('status', 1)->first();
                if($author) {
                    $pdfs =  $author->pdf()->where('status', 1)->latest()->get();
                }else{
                    $series = SeriesPdf::where('name','LIKE',"%{$search}%")->where('status', 1)->first();
                    if($series) {
                        $pdfs =  $series->pdf()->where('status', 1)->latest()->get();
                    }
                }
            }
        }
        ...
        // STORY
        $blogs = Blog::where('title','LIKE', "%{$search}%")->where('status', 1)->latest()->get();
        if (count($blogs) == 0) {
            $category = CategoryBlog::where('name','LIKE', "%{$search}%")->where('status', 1)->first();
            if($category) {
                $blogs = $category->blog()->where('status', 1)->latest()->get();
            }
        }
