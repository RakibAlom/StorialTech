...
<div id="content" class="main-content">
    <div class="layout-px-spacing">
        ...
        <div class="page-header">
            ...
            @if(URL::current() == route('admin.movie'))
                <h3>MOVIE LIST</h3>
            @elseif(URL::current() == route('admin.deactive-list.movie'))
                <h3>DEACTIVE MOVIE LIST</h3>
            @endif
            </div>
        </div>
                        ...
                        <table id="html5-extension" class="table table-hover non-hover" style="width:100%">
                            ...
                            @foreach($movies as $item)
                                <tr>
                                    <td>{{ $count++ }}</td>
                                    <td>
                                        <a href="{{ route('admin.show.movie',$item->slug) }}">{{ $item->name }}</a><br>
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            <a href="{{ route('admin.show.movie',$item->slug) }}" class="btn btn-info btn-sm">
                                            ...
                                            <div class="dropdown-menu" aria-labelledby="dropdownMenuReference1">
                                              <a class="dropdown-item" href="{{ route('admin.edit.movie', $item->id) }}">Edit</a>
                                            @if($item->status == 2)
                                              <a class="dropdown-item active-alert" href="{{ route('admin.active.movie', $item->id) }}">Active</a>
                                            @else
                                              <a class="dropdown-item deactive-alert" href="{{ route('admin.deactive.movie', $item->id) }}">Deactive</a>
                                            @endif
                                              <div class="dropdown-divider"></div>
                                              <a class="dropdown-item trash-alert" href="{{ route('admin.soft-delete.movie', $item->id) }}">Delete</a>
                                            </div>
                                          </div>
                                    </td>
                                </tr>
...
<!-- NOTE TO Use Copy CSV Excel movie Print Options You Must Include These Files  -->
