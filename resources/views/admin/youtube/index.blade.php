...
<div id="content" class="main-content">
    <div class="layout-px-spacing">
        ...
        <div class="page-header">
            ...
            @if(URL::current() == route('admin.youtube'))
                <h3>YouTube Channel List</h3>
            @elseif(URL::current() == route('admin.deactive-list.youtube'))
                <h3>Deactive YouTube Channel List</h3>
            @endif
            </div>
        </div>
                        ...
                        <table id="html5-extension" class="table table-hover non-hover" style="width:100%">
                            ...
                            @foreach($youtubes as $item)
                                <tr>
                                    <td>{{ $count++ }}</td>
                                    <td>
                                        <a href="{{ route('admin.show.youtube',$item->slug) }}">{{ $item->name }}</a><br>
                                        <small>({{ $item->slug }})</small>
                                    </td>
                                    <td>{{ $item->elink }}</td>
                                    <td><a href="{{ $item->clink }}" target="_blank">{{ $item->clink }}</a></td>
                                    <td>
                                    @if($item->image)
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            <a href="{{ route('admin.show.youtube',$item->slug) }}" class="btn btn-info btn-sm">
                                            ...
                                            <div class="dropdown-menu" aria-labelledby="dropdownMenuReference1">
                                              <a class="dropdown-item" href="{{ route('admin.edit.youtube', $item->id) }}">Edit</a>
                                            @if($item->status == 0)
                                              <a class="dropdown-item active-alert" href="{{ route('admin.active.youtube', $item->id) }}">Active</a>
                                            @else
                                              <a class="dropdown-item deactive-alert" href="{{ route('admin.deactive.youtube', $item->id) }}">Deactive</a>
                                            @endif
                                              <div class="dropdown-divider"></div>
                                              <a class="dropdown-item trash-alert" href="{{ route('admin.soft-delete.youtube', $item->id) }}">Delete</a>
                                            </div>
                                          </div>
                                    </td>
                                </tr>
