            ...
            <div id="navSection" data-spy="affix" class="nav sidenav">
                <div class="sidenav-content">
                    ...
                    <a href="#status" class="nav-link">
                        Status:
                        @if($story->status == 0)
                            <a class="btn btn-success approve-alert" href="{{ route('admin.approve.story', $story->id) }}">Approve</a>
                        @elseif($story->status == 1)
                            Active
                        @elseif($story->status == 2)
                            Deactived
                        @elseif($story->status == 9)
                            Story in Trash
                        @endif
                    </a>
                </div>
                ...
                <div id="heading" class="col-xl-12 col-lg-12 layout-spacing">
                    ...
                    <div class="statbox widget box box-shadow">
                        ...
                        <div class="widget-content widget-content-area">
                            @if($story->image)
                                <img class="img-thumbnail" src="{{ asset('storage/app/public/'.$story->image) }}" alt="{{ $story->title }}">
                            @endif

                            <h6 class="mt-4">Category:
                                @foreach($story->category as $category)
                                <a href="#" class="text-success">{{ $category->categorystory->name }}</a>,
                                @endforeach
                                <span class="float-right">{{ $story->created_at->diffForHumans() }}</span>
                            </h6>
                            <hr>
                            ...
                            @endphp
                            @if($like)
                                <a class="btn btn-danger btn-sm unlike" href="{{ route('unlike.story', $user->storylike->id) }}">Liked
                                    @if($story->like)
                                        ({{ $story->like->count() }})
                                    @endif
                                </a>
                            @else
                                <a class="btn btn-primary btn-sm like" href="{{ route('like.story', $story->id) }}">Like
                                    @if($story->like)
                                        ({{ $story->like->count() }})
                                    @endif
                                </a>
                            @endif --}}
                            <span class="btn btn-primary btn-sm like">Likes ({{ $story->like->count() }})</span>
                            </div>
                        </div>
                    </div>
                </div>
