...
<div id="content" class="main-content">
    <div class="container">
        ...
        <div class="container">
                ...
                <div class="sidenav-content">
                    ...
                    <a href="#status" class="nav-link">
                        ...
                        @if($tutorial->status == 0)
                            <a class="btn btn-success approve-alert" href="{{ route('admin.approve.tutorial', $tutorial->id) }}">Approve</a>
                        @elseif($tutorial->status == 1)
                            Active
                        @elseif($tutorial->status == 2)
                            Deactived
                        @elseif($tutorial->status == 9)
                            tutorial in Trash
                        @endif
                    </a>
                </div>
                ...
                <div id="heading" class="col-xl-12 col-lg-12 layout-spacing">
                    ...
                    <div class="statbox widget box box-shadow">
                        ...
                        <div class="widget-content widget-content-area text-white">
                            @if($tutorial->image)
                                <img class="img-thumbnail" src="{{ asset('storage/app/public/'.$tutorial->image) }}" alt="{{ $tutorial->title }}">
                            @endif

                            <h6 class="mt-4">Category:
                                @foreach($tutorial->category as $category)
                                <a href="#" class="text-success">{{ $category->categorytutorial->name }}</a>,
                                @endforeach
                                <span class="float-right">{{ $tutorial->created_at->diffForHumans() }}</span>
                            </h6>
                            <hr>
                            ...
                            @endphp
                            @if($like)
                                <a class="btn btn-danger btn-sm unlike" href="{{ route('unlike.tutorial', $user->tutoriallike->id) }}">Liked
                                    @if($tutorial->like)
                                        ({{ $tutorial->like->count() }})
                                    @endif
                                </a>
                            @else
                                <a class="btn btn-primary btn-sm like" href="{{ route('like.tutorial', $tutorial->id) }}">Like
                                    @if($tutorial->like)
                                        ({{ $tutorial->like->count() }})
                                    @endif
                                </a>
                            @endif --}}
                            <span class="btn btn-primary btn-sm like">Likes ({{ $tutorial->like->count() }})</span>
            ...
            @include('admin.tutorial.comment')
