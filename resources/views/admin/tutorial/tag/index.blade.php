                        ...
                        <table id="html5-extension" class="table table-hover non-hover" style="width:100%">
                            ...
                            @foreach($tags as $item)
                                <tr>
                                    <td>{{ $count++ }}</td>
                                    <td>{{ $item->name }}</td>
                                    <td>{{ $item->categorytutorial->name }}</td>
                                    <td>{{ $item->slug }}</td>
                                    <td>
                                        @if($item->status == 0)
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            <a href="{{ route('admin.edit-tag.tutorial',$item->id) }}" class="btn btn-info btn-sm">
                                            ...
                                            @if($item->status == 0)
                                              <a class="dropdown-item active-alert" href="{{ route('admin.active-tag.tutorial', $item->id) }}">Active</a>
                                            @else
                                              <a class="dropdown-item deactive-alert" href="{{ route('admin.deactive-tag.tutorial', $item->id) }}">Deactive</a>
                                            @endif
                                              <div class="dropdown-divider"></div>
                                              <a class="dropdown-item trash-alert" href="{{ route('admin.soft-delete-tag.tutorial', $item->id) }}">Delete</a>
                                            </div>
                                          </div>
                                    </td>
                                </tr>
